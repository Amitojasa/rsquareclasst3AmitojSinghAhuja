...
def student_guardian(request, pk):
    ...
    if request.method == 'POST':
        ...
        mStudent = models.StudentProfile.objects.get(student_id=student.id)
        mGuardians = models.Guardian.objects.filter(student_id=mStudent.id)
          

        return render(request, 'student/guardian_form.html',{'guardians':mGuardians})
